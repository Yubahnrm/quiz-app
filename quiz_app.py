...
st.markdown("""
    ...
    .correct { color: #27ae60; font-size: 1.3em; font-weight: bold; }
    .wrong { color: #e74c3c; font-size: 1.3em; font-weight: bold; }
    .explanation { background: #eaf4fb; padding: 10px; border-radius: 8px;
                   color: #2980b9; margin-top: 8px; }
...
def generate_quiz(topic, num_questions, language, difficulty, quiz_type):
    quiz_maker = Agent(
        role="Quiz Maker",
        goal="Create quiz questions for students",
        backstory="You are an expert teacher who creates clear and fun quiz questions.",
        llm="groq/llama-3.3-70b-versatile"
    )
    task = Task(
        description=f"""Create {num_questions} {quiz_type} questions about {topic} in {language} language.
...
Format each question clearly and consistently.""",
        expected_output=f"""{num_questions} questions each with options, correct answer and explanation.
...
Explanation: [why this is correct]""",
    ...
    crew = Crew(agents=[quiz_maker], tasks=[task])
    result = crew.kickoff()
    return str(result)
...
# ════════════════════════════════
    ...
    if current >= total:
        ...
        # Show feedback from previous submission if any
